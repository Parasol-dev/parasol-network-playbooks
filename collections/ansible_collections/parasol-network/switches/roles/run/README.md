Parasol-network.Switches Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: parasol-network.switches.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from parasol-network.switches
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: parasol-network.switches.run
      vars:
        run_x: 42
```
